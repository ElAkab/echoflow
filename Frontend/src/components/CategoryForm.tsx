...
export default function CategoryForm({ onSuccess }: CategoryFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 bg-white p-6 rounded-lg border">
      ...
      <div>
        <label htmlFor="ai_system_context" className="block text-sm font-medium mb-1">
          AI System Context
        </label>
        <textarea
          id="ai_system_context"
          {...register('ai_system_context')}
          className="w-full border rounded px-3 py-2"
          rows={4}
          placeholder="Instructions for AI when processing notes in this category..."
        />
        {errors.ai_system_context && (
          <p className="text-red-500 text-sm mt-1">{errors.ai_system_context.message}</p>
        )}
      </div>
